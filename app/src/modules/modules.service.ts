...
import { ModuleType } from './dto/save-order.dto';
...
export class ModulesService {
  ...
  /**
   * Save order for modules or lessons within a course
   */
  async saveOrder(
    ...
    this.logger.log(`Saving order for ${saveOrderDto.moduleType}s: ${JSON.stringify(saveOrderDto)}`);
    ...
    try {
      ...
      const result = await this.moduleRepository.manager.transaction(async (transactionalEntityManager) => {
        if (saveOrderDto.moduleType === ModuleType.MODULE) {
          // Validate courseId is provided for module ordering
          if (!saveOrderDto.courseId) {
            throw new BadRequestException(RESPONSE_MESSAGES.ERROR.COURSE_ID_REQUIRED);
          }

          // Save order for modules within a course
          await this.saveModuleOrder(
            saveOrderDto.courseId,
            saveOrderDto.ids,
            userId,
            tenantId,
            organisationId,
        ...
        } else if (saveOrderDto.moduleType === ModuleType.LESSON) {
          // Validate moduleId is provided for lesson ordering
          if (!saveOrderDto.moduleId) {
            throw new BadRequestException(RESPONSE_MESSAGES.ERROR.MODULE_ID_REQUIRED);
          }

          // Save order for lessons within a module
          await this.saveLessonOrder(
            saveOrderDto.courseId,
            saveOrderDto.moduleId,
            saveOrderDto.ids,
            userId,
            tenantId,
            organisationId,
        ...
        return { success: true, message: `${saveOrderDto.moduleType} order ${RESPONSE_MESSAGES.ORDER_SAVED_SUCCESSFULLY}` };
      });

      // Handle cache invalidation after successful transaction
      if (this.cache_enabled) {
          const courseModuleCacheKey = `${this.cache_prefix_module}:course:${saveOrderDto.courseId}:${tenantId}:${organisationId}`;
          const courseHierarchyCacheKey = `${this.cache_prefix_course}:hierarchy:${saveOrderDto.courseId}:${tenantId}:${organisationId}`;
      ...
      this.logger.error(`Error saving order for ${saveOrderDto.moduleType}s: ${error.message}`, error.stack);
      ...
      if (error instanceof BadRequestException) {
        ...
        throw new BadRequestException(`${RESPONSE_MESSAGES.ERROR.SAVING_ORDER_FAILED} ${saveOrderDto.moduleType}s: ${error.message}`);
      }
    }
  }
  ...
  private async saveModuleOrder(
    ...
    try {
      ...
      // Validate that all modules exist and belong to the course
      ...
      // Validate that all modules are top-level modules (no parentId)
      const modulesWithParent = modules.filter(m => m.parentId !== null);
      if (modulesWithParent.length > 0) {
        throw new BadRequestException(RESPONSE_MESSAGES.ERROR.CANNOT_ORDER_SUBMODULES);
      }

      // Update ordering for each module based on its position in the array
      const updatePromises = moduleIds.map((moduleId, index) => {
        return transactionalEntityManager.update(Module, { moduleId }, {
          ordering: index,
          updatedBy: userId,
          updatedAt: new Date()
        });
      ...
      this.logger.log(`Successfully saved order for ${modules.length} modules in course ${courseId}`);
    } catch (error) {
      this.logger.error(`Error in saveModuleOrder: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  private async saveLessonOrder(
    ...
    try {
      ...
      const module = await transactionalEntityManager.findOne(Module, {
        where: {
          moduleId,
          courseId,
          status: Not(ModuleStatus.ARCHIVED),
      ...
      // Validate that all lessons exist and belong to the module
      ...
      if (lessons.length !== lessonIds.length) {
        throw new BadRequestException(RESPONSE_MESSAGES.ERROR.SOME_LESSONS_NOT_FOUND);
      }

      // Update ordering for each lesson based on its position in the array
      const updatePromises = lessonIds.map((lessonId, index) => {
        return transactionalEntityManager.update(Lesson, { lessonId }, {
          ordering: index,
          updatedBy: userId,
          updatedAt: new Date()
        });
      ...
      this.logger.log(`Successfully saved order for ${lessons.length} lessons in module ${moduleId}`);
    } catch (error) {
      this.logger.error(`Error in saveLessonOrder: ${error.message}`, error.stack);
      throw error;
    }
  }
}